...
int main(int argc, char *argv[])
{
    ...
    if (argv[2] != NULL && strcmp(argv[2], "time") == 0)
    {
        timing = TRUE;
    }
    ...
    uint32_t *pixel_buffer = malloc((ECRAN_HAUTEUR * ECRAN_LARGEUR) * sizeof(uint32_t));
    ...
    while (user_chip8.executionFlag)
    {
        ...
        // in this loop until the user clears the pauseFlag or presses the esc key
        do
        {
            process_user_input(&user_chip8);
        } while (user_chip8.pauseFlag && user_chip8.executionFlag);
